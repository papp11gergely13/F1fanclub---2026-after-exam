...
<head>
    ...
    <title>Futam Eredmények - F1 Fan Club</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Qualifying specific styles - FULL WIDTH */
...
<body>
        ...
        <div class="championship-content full-width">
            ...
            <div class="control-panel">
                <div class="panel-header">
                    ...
                    <h2>FUTAM <span>EREDMÉNYEK</span></h2>
                </div>
                ...
                <div class="current-selection">
                    <div class="selection-info">
                        ...
                        <span class="selection-type">
                            <span id="selectedType">FUTAM</span> SZEZON
                        </span>
                    </div>
            ...
            <div class="standings-card">
                <div class="table-header">
                    <h2>
                        ...
                        <span id="tableTitle">FUTAM EREDMÉNYEK</span>
                    </h2>
                    ...
                    <table class="f1-table">
                        <thead>
                            <tr>
                                ...
                                <th>VERSENYZŐ</th>
                                <th>CSAPAT</th>
                                <th style="text-align:center;">KÖRÖK</th>
                                <th style="text-align:center;">IDŐ / KIESÉS</th>
                                <th style="text-align:center;">PONTOK</th>
                            </tr>
                        </thead>
    ...
    <script>
        ...
        // COMPLETE TEAM COLOR MAPPING - ALL TEAMS (matching PHP getTeamColor)
        ...
        document.addEventListener('DOMContentLoaded', () => {
                ...
                fetch(`https://api.jolpi.ca/ergast/f1/${year}.json`)
                    ...
                    .then(data => {
                        ...
                        if (races && races.length > 0) {
                            ...
                            loadRaceResults(currentYear, currentRound, raceSelect.options[raceSelect.selectedIndex].text);
                        } else {
            ...
            updateBtn.addEventListener('click', () => {
                if(!raceSelect.disabled) {
                    ...
                    loadRaceResults(currentYear, currentRound, raceName);
                }
            });
            ...
            // FETCH RACE RESULTS INSTEAD OF QUALIFYING
            function loadRaceResults(year, round, raceName) {
                const cleanRaceName = raceName.replace(/F\d+ - /, '');
                tableTitle.innerHTML = `${cleanRaceName} <span style="color:#666;">|</span> FUTAM`;

                standingsBody.innerHTML = '<tr><td colspan="6" class="loading-message"><i class="fas fa-circle-notch fa-spin"></i> Futam adatok betöltése...</td></tr>';

                // Notice: 'results.json' instead of 'qualifying.json'
                fetch(`https://api.jolpi.ca/ergast/f1/${year}/${round}/results.json`)
                    ...
                    .then(data => {
                        ...
                        if (!races || races.length === 0 || !races[0].Results) {
                            standingsBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:40px; color:#666;">Nem találtunk futam eredményt ehhez a nagydíjhoz. Lehet, hogy még nem rendezték meg.</td></tr>';
                            return;
                        }

                        const list = races[0].Results;
                        let html = '';

                        list.forEach(res => {
                            const isWinner = res.position === "1";
                            const teamName = res.Constructor?.name || "Ismeretlen";
                            const teamColor = getTeamColorJS(teamName);
                            
                            const rowClass = isWinner ? 'f1-champion' : '';
                            const gradientStyle = `linear-gradient(90deg, ${teamColor}20 0%, transparent 40%)`;
                            const trophy = isWinner ? '<i class="fas fa-trophy" style="color:#d4af37; margin-right:8px;"></i>' : '';
                            
                            // Get Laps
                            const laps = `<span class="lap-time">${res.laps}</span>`;
                            
                            // Get Time OR Status (If they DNF'd or were lapped)
                            let timeOrStatus = '';
                            if (res.Time && res.Time.time) {
                                timeOrStatus = `<span class="lap-time">${res.Time.time}</span>`;
                            } else {
                                timeOrStatus = `<span class="lap-time time-miss">${res.status}</span>`;
                            }

                            // Get Points
                            const pointsValue = res.points > 0 ? `+${res.points}` : res.points;
                            const pointsColor = res.points > 0 ? '#28a745' : '#666'; // Zöld, ha kapott pontot
                            const pointsHtml = `<span class="lap-time" style="color:${pointsColor}; font-weight:700;">${pointsValue}</span>`;

                            html += `
                                <tr class="${rowClass}" style="background: ${gradientStyle};">
                                    <td class="f1-pos" ${isWinner ? 'style="color:#d4af37;"' : ''}>${res.position}</td>
                                    <td class="f1-name">${trophy}${res.Driver.givenName} <span>${res.Driver.familyName}</span></td>
                                    <td class="f1-team" style="color: ${teamColor};">${teamName}</td>
                                    <td style="text-align:center;">${laps}</td>
                                    <td style="text-align:center;">${timeOrStatus}</td>
                                    <td style="text-align:center;">${pointsHtml}</td>
                                </tr>
                            `;
                        });
